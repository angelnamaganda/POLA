...
class OtherReports extends StatefulWidget {
  const OtherReports({super.key});

  @override
  State<OtherReports> createState() => _ReportsState();
}

class _ReportsState extends State<OtherReports> {
  ...
  Future<void> getCurrentUserName() async{
   ...
   void savePost() async{
    ...
    final Reference storageReference = FirebaseStorage.instance.ref().child('other_image/${DateTime.now()}.jpg');
    UploadTask uploadTask = storageReference.putFile(_selectedImage!);
    TaskSnapshot storageTaskSnapshot = await uploadTask.whenComplete(() => {});

    String imageUrl = await storageTaskSnapshot.ref.getDownloadURL();

    await FirebaseFirestore.instance.collection('other_reports').add({
      'username' : username,
   ...
   //show a snackbar to provide feedback to the user.
    if(mounted){
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Other Report Submitted Successfully!'),
